...
import com.netmania.checklod.org.adapter.BeaconListAdapterForJob;
import com.netmania.checklod.org.data.DataProviderUtil;
import com.netmania.checklod.org.manage.FlagBox;
import com.netmania.checklod.org.utils.PermissionUtilsM;
...
public class ActivityJobMain
    ...
    private JavisForCheckLOD myJavis;
    ...
    public void onRequestPermissionsResult (int requestCode, String[] permissions, int[] grantresults) {
        ...
        switch (requestCode) {
            ...
            case FlagBox.PERMISSION_REQ_JAVIS :
                ...
                if (AllPermissionGranted) {
                    //initJavis();
                    mHandler.post(new InfoToast(mActivity, getResources().getString(R.string.toast_javis_permission)));
                } else {
                    //nothing yet;
                }
    ...
    private void initData () {
        ...
        if (items.size() > 0) {
            ...
            for (int i = 0; i < items.size(); i++) {
                ...
                /** 프루브 측정이 안될 경우 칩셋온도로 고고싱!!!! ---------------------------------------------------------------------------------------------------- */
                boolean hasProbeTemp = true;//DataProviderUtil.getInstance(mApp).isProbeMode(items.get(i).MAC);
                if (hasProbeTemp) {
                    items.get(i).$max_temp = DataProviderUtil.getInstance(mApp).getSavedMaxTemp(items.get(i).MAC, true);
                    items.get(i).$min_temp = DataProviderUtil.getInstance(mApp).getSavedMinTemp(items.get(i).MAC, true);
                } else {
                    items.get(i).$max_temp = DataProviderUtil.getInstance(mApp).getSavedMaxTemp(items.get(i).MAC, false);
                    items.get(i).$min_temp = DataProviderUtil.getInstance(mApp).getSavedMinTemp(items.get(i).MAC, false);
                }
                /** 프루브 측정이 안될 경우 칩셋온도로 고고싱!!!! ---------------------------------------------------------------------------------------------------- */
     ...
     *
     */
    private void initJavis () {

        if (!PermissionUtilsM.checkJavisPermission(mActivity)) {
            return;
        }

        if (myJavis == null) myJavis = JavisForCheckLOD.getInstance(mActivity, mJavisCallback);
    }
    ...
    public void onResume () {
        ...
        if (myJavis != null) myJavis.onResume();
    }
    ...
    public void onDestroy () {
        super.onDestroy();
        if (myJavis != null) myJavis.onDestroy();
    }
    ...
    private void addTestDevices () {
        ...
        for (int i = 0; i < targets.length; i++) {
            ...
            new DBHelper(BaseApplication.getInstance()).update(BeaconItemDto.getInstance().getUpdateTempRageQry(targets[i], 10, 40));
        }
     ...
     *
     */
    private JavisForCheckLOD.OnJavisCallback mJavisCallback = new JavisForCheckLOD.OnJavisCallback() {
        @Override
        public void onJavisReady() {
            //nothing yet;
        }

        @Override
        public void onBeaconAdd() {
            ActivityControl.openUserInputUI(mActivity.getIntent(), mActivity, FlagBox.MODE_ADD_DEVICE);
        }
    };
}
