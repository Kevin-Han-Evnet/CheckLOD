...
import com.netmania.checklod.org.adapter.TemperReportListAdapter;
...
public class ActivityJobReport
    ...
    private void setLayout () {
        ...
        if (!Constants.IS_RELEASED) fv_item_max_temperature.setOnLongClickListener(this);
        ...
        if (!Constants.IS_RELEASED) item_max_temperature.setOnLongClickListener(this);
    ...
    private void initData () {
        ...
        boolean hasProbeTemp = true;//com.netmania.checklod.org.data.DataProviderUtil.getInstance(mApp).isProbeMode(MAC);
        if (hasProbeTemp) {
            maxTemp = DataProviderUtil.getInstance(mApp.getApplicationContext()).getSavedMaxTemp(MAC, true);
            minTemp = DataProviderUtil.getInstance(mApp.getApplicationContext()).getSavedMinTemp(MAC, true);
        } else {
            maxTemp = DataProviderUtil.getInstance(mApp.getApplicationContext()).getSavedMaxTemp(MAC, false);
            minTemp = DataProviderUtil.getInstance(mApp.getApplicationContext()).getSavedMinTemp(MAC, false);
        }
        /** 프루브 측정이 안될 경우 칩셋온도로 고고싱!!!! ---------------------------------------------------------------------------------------------------- */
    ...
    private void setData(LineChart mChart, final ArrayList<TemperatureTrackingDto> datas) {
        ...
        if (datas != null && datas.size() > 0)
            for (int i = 0; i < datas.size(); i++) {
                ...
                boolean hasProbeTemp = true;//com.netmania.checklod.org.data.DataProviderUtil.getInstance(mApp).isProbeMode(MAC);
                if (!hasProbeTemp) {
                    datas.get(i).temp = datas.get(i).outside_temp;
                    datas.get(i).hum = datas.get(i).outside_hum;
                }
                /** 프루브 측정이 안될 경우 칩셋온도로 고고싱!!!! ---------------------------------------------------------------------------------------------------- */
        ...
        DialogUtils.alert(
                ...
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        ...
                        String fileName = (file != null) ? file.getName() : "no image";
                        resultData.putExtra(EXTRA_KEY_PHOTO, fileName);
    ...
    public boolean onLongClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.item_max_temperature :
                onCheckOutSuccess (MAC);
                break;
    ...
    public void onCheckOutSuccess(final String mac) {
        if (progressGo != null) progressGo.setVisibility(View.GONE);
        DialogUtils.alert(
                mActivity,
